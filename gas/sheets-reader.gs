/**
 * 【v5以降：本番では未使用】
 * 他研修スケジュールスプシ（2026年／2027年）から読み取るための旧ロジック。
 *
 * v5で bootstrapHandler から呼び出しを外し、クライアントの
 * assets/data/other-trainings.json を唯一の情報源とする運用に切替。
 * これにより他研修スプシは「一度だけコピーした情報」となり、
 * あちら側が更新されても自動反映はされない（独立運用）。
 *
 * 再度スプシからスナップショットを取り直したい場合は、
 * ルートの _snapshot_other_trainings.py を実行して JSON を更新する。
 * （このファイル内の関数は、そのスクリプトから呼ばれる doGet パスでのみ使われる可能性を残す）
 *
 *   - A: 日付(MM/DD) / B: 曜日 / C: 祝日名
 *   - D: PBC / E: 顧問 / F: 東京プログラム / G: 浜松プログラム / H: 静岡プログラム
 */
